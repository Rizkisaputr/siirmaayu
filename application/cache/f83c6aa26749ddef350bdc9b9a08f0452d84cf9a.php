...
<nav class="navbar navbar-light bg-gradient-aqua justify-content-between">
	<div class="container">
		<a class="navbar-brand">
			...
			<h4 align="center"><b><font size='4' color='#03b290'>Peta GIS Fasilitas Kesehatan <br> <?php echo e($application_copyright); ?> </font></b></h4>
							<div class="live"> <div class="live-animate"><div style="width:12px; height:12px"></div></div>
							<h5 style="font-size: 15px;">Live Update</h5>
							</div>
		<form class="form-inline" method="GET" action="<?php echo e(base_url('auth/login')); ?>">
...
<footer class="m-2 text-center">
	<p style="font-size: 14px;">Copyright © 2013-2020 <?php echo e($application_copyright); ?></p>
</footer>
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<p class="text-right mb-0" style="font-size: 14px;">© 2013-2020 <?php echo e($application_copyright); ?></p>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	$(function(){
		...
		$('#exampleModal').on('show.bs.modal',function(event){
			...
			$('#detail-rs').load("<?php echo e(base_url('map/rs_detil')); ?>/"+idrs,'',function(res,stat,xhr){
		...
		var marker, count;
		$.get('<?php echo e(base_url('map/rs_map')); ?>',function(res){
			console.log(res);
	...
	function refresh_bed() {
		/*('#list_bed').load('<?php echo e(base_url('map/bed_data')); ?>','',function(response, status, xhr){
			setTimeout(refresh_bed,50000);
		});*/
		$.get('<?php echo e(base_url('map/bed_data')); ?>', function (data) {
			$('#list_bed1').html(data);
		});
		setInterval(function() {
	        $.get('<?php echo e(base_url('map/bed_data')); ?>', function (data) {
	...
	function refresh_sms() 
	{
		setInterval(function() {
			$.ajax({
				type: "GET",
				url:'<?php echo e(site_url('map/api_sms')); ?>',
				dataType: "json",
				success: function(res) {
					$.ajax({
	                    url: '<?php echo e(base_url('map/process_sms')); ?>',
	                    type: "POST",
	                    data: res,
	                    cache: false,
	                    dataType: "json",
	...
	function refresh_wa() 
	{
		setInterval(function() {
			$.ajax({
				type: "GET",
				url:'<?php echo e(site_url('map/api_wa')); ?>',
				dataType: "json",
				success: function(res) {
					$.ajax({
	                    url: '<?php echo e(base_url('map/process_wa')); ?>',
	                    type: "POST",
	                    data: res,
	                    cache: false,
	                    dataType: "json",
