...
class DG_covid extends DataGridModelBase
{
	private $where;
	public function __construct()
	{
		parent::__construct('tb_covid', array('tb_pasien.nama','tb_pasien.goldarah','tb_rs.nama','tb_covid.type','tb_covid.dibuat','tb_covid.alasan_rujukan','tb_covid.status_rujukan'));
	...
	protected function select_query_obj()
	{
		...
		return $this
			->db
			->select('tb_covid.type,tb_covid.id_rujukan,tb_pasien.nama AS pasien,tb_pasien.umur AS umur, tb_pasien.pasangan as pasangan, tb_pasien.goldarah as goldarah, tb_rs.nama AS rs,p.nama AS perujuk, tb_covid.dibuat,tb_covid.alasan_rujukan,tb_covid.pembiayaan,tb_covid.tindakan,tb_covid.media,tb_covid.transportasi, tb_covid.no_rm,tb_covid.status_rujukan,penerima.full_name,pengalih.nama as pengalih,tb_covid.info_rujuk_balik, tb_covid.diagnosis,tb_covid.kesadaran,tb_covid.tensi,tb_covid.nadi,tb_covid.suhu,tb_covid.pernapasan,tb_covid.nyeri,tb_covid.ibuanak_namabidan,tb_covid.ibuanak_nobidan,tb_covid.rujukbalik_status')
			->join('tb_rs p','tb_covid.id_rs_perujuk = p.id_rs')
			->join('tb_rs','tb_covid.id_rs_dirujuk = tb_rs.id_rs')
			->join('tb_rs pengalih','pengalih.id_rs = tb_covid.id_rs_pengalih','left')
			->join('users as penerima','penerima.id=tb_covid.id_penerima','left')
			->join('tb_pasien','tb_covid.no_rm = tb_pasien.id_rm');
	}
	...
	protected function export_row_query($args)
	{
		...
		if(!is_null($args['start'])&&!is_null($args['end'])){
			$start	= $args['start'];
			$end	= $args['end'];
			$this->db->where("DATE(tb_covid.dibuat) BETWEEN '$start' AND '$end'",NULL,FALSE);
		}
		$this->db->select('tb_pasien.nama as pasien,tb_pasien.umur as umur,tb_pasien.pasangan as pasangan,tb_pasien.goldarah as goldarah, perujuk.nama as perujuk,dirujuk.nama as dirujuk,tb_covid.media,tb_covid.type,tb_covid.no_rm, tb_covid.alasan_rujukan,tb_covid.pembiayaan,tb_covid.transportasi,tb_covid.no_rm,tb_covid.diagnosis,tb_covid.ibuanak_icdx,tb_covid.kesadaran,tb_covid.tensi,tb_covid.nadi,tb_covid.suhu,tb_covid.pernapasan,tb_covid.nyeri,tb_covid.keterangan_lain,tb_covid.hasil_lab,tb_covid.hasil_radiologi_ekg,tb_covid.tindakan,tb_covid.status_rujukan,tb_covid.info_rujuk_balik,tb_covid.dibuat,tb_covid.direspon,tb_covid.ibuanak_namabidan,tb_covid.ibuanak_nobidan,tb_layanan_type.nama AS layanan,tb_kelas_bed.nama as kelas_bed');
		$this->db->join('tb_rs perujuk','perujuk.id_rs=tb_covid.id_rs_perujuk');
		$this->db->join('tb_rs dirujuk','dirujuk.id_rs=tb_covid.id_rs_dirujuk');
		$this->db->join('tb_pasien','tb_covid.no_rm=tb_pasien.id_rm');
		$this->db->join('tb_kelas_bed','tb_covid.id_kelas_bed=tb_kelas_bed.id_kelas_bed','left');
		$this->db->join('tb_layanan_type','tb_covid.id_jenis_layanan=tb_layanan_type.id_jenis_layanan','left');
	...
	public   function  get_hp($ibuanak_nobidan)  {
		...
		//$ibuanak_nobidan = $this->db->get('tb_covid');
