...
class User_model extends CI_Model
{
	...
	public function is_puskesmas($uid){
		...
		->join('users','users.id = tb_rs_permission.id_user')
		->join('users_groups','users_groups.user_id = users.id')
		->join('groups','groups.id = users_groups.group_id')
		->select('tb_rs.jenis,groups.id')
		->where('tb_rs_permission.id_user',$uid)
		->where('groups.id',3)
		->get();
	...
	public function is_rs($uid){
		...
		foreach ($rs->result() as $row){
			if ($row->jenis == "Rumah Sakit") $is_rs = TRUE;
		}
		return $is_rs;
	}

	public function is_rs2($uid){
		$is_rs2 = FALSE;
		if($this->ion_auth->is_rs2($uid))
			return TRUE;
		$this->db->reset_query();
		$ids=array();
		$rs = $this->db->from('tb_rs_permission')
		...
		->get();
		foreach ($rs->result() as $row){
			if ($row->jenis == "Rumah Sakit") $is_rs2 = TRUE;
		}
		return $is_rs2;
	}
}
